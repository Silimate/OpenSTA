...
define_var_help sta_sdc_name_folding {0|1} \
  {When a `get_cells`, `get_pins`, `get_nets` or `get_ports` pattern (or an implicit object name in an SDC command) matches nothing, retry with every run of the characters `/ . [ ] _` folded to a single `_` in both the pattern and the netlist names, so names flattened by a synthesis tool (`gen_0_child`, `wrapper_child`) find the elaborated hierarchical objects (`gen[0].child`, `wrapper/child`). A pin on a register may also match a register instance name that carries one extra bit index (`reg` finds `reg[0]`), a register of a multi-dimensional array may be named with `_reg` before a different index (`mem_reg[1][2]` finds `mem[1]_reg[2]` and the reverse), a clock pin name missing on a register or latch with a single clock pin maps to that pin (`CP` finds `CLK`), and a port glob that matches no port name is matched against bus bit names. A name without wildcards is only resolved when exactly one object matches; ambiguous names stay unresolved and the candidates are listed. Every resolution is reported with warning 2740 or 2741. The default value is 1.}
