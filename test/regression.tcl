...
proc setup {} {
  global result_dir diff_file failure_file errors tests_run
...
proc run_test { test {collections 0} } {
  global result_dir diff_file errors diff_options report_stats tests_run
  ...
  if [file exists $cmd_file] {
    ...
    set log_file [test_log_file $test $collections]
    file delete -force $log_file
    if { $collections } {
      puts -nonewline "$test (collections)"
    } else {
      puts -nonewline $test
    }
    flush stdout
    set test_errors [run_test_app $test $cmd_file $log_file $collections]
...
# Return error.
proc run_test_app { test cmd_file log_file {collections 0} } {
  global app_path errorCode use_valgrind
  if { $use_valgrind } {
    return [run_test_valgrind $test $cmd_file $log_file $collections]
  } else {
    return [run_test_plain $test $cmd_file $log_file $collections]
  }
}

proc run_test_plain { test cmd_file log_file {collections 0} } {
  ...
  if { ![file exists $app_path] } {
    ...
    puts $run_stream "set ::sta_enable_collections $collections"
...
proc run_test_valgrind { test cmd_file log_file {collections 0}} {
  ...
  puts $vg_stream "set ::sta_enable_collections $collections"
...
proc show_summary {} {
  global errors tests diff_file result_dir valgrind_shared_lib_failure tests_run
  global app_path app
  
  puts "------------------------------------------------------"
  if { [found_errors] } {
    if { $errors(error) != 0 } {
      puts "Errored $errors(error)/$tests_run"
    }
    if { $errors(fail) != 0 } {
      puts "Failed $errors(fail)/$tests_run"
    }
    if { $errors(leak) != 0 } {
      puts "Memory leaks in $errors(leak)/$tests_run"
    }
    if { $errors(memory) != 0 } {
      puts "Memory corruption in $errors(memory)/$tests_run"
    }
    if { $errors(no_ok) != 0 } {
      puts "No ok file for $errors(no_ok)/$tests_run"
    }
    if { $errors(no_cmd) != 0 } {
      puts "No cmd tcl file for $errors(no_cmd)/$tests_run"
    }
    if { $errors(fail) != 0 } {
      puts "See $diff_file for differences"
    }
  } else {
    puts "Passed $tests_run"
  }
...
proc test_log_file { test {collections 0}} {
  global result_dir
  if { $collections } {
    return [file join $result_dir "$test.collections.log"]
  } else {
